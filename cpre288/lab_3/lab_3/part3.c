...
void objectDetection(float distances[], char message[], cyBOT_Scan_t scan){
    ...
    for (i = 3; i < 90 - 1; i++){
           if (distances[i] < 70 && fabs(distances[i] - distances[i + 1]) < 4){
               ...
               endAngle = i * 2;
               sprintf(message, "%d   %3.3f  %3.3f",(startAngle + endAngle) / 2 , objectDistance, ((endAngle - startAngle) / 360.0) * 2 * 3.1415 * objectDistance);
...
void cleanData(float uncleanData[], char message[], cyBOT_Scan_t scan){
    ...
    for (i = 0; i < 90 - 3; i++){
        //old method basically just took a rolling average which made object detection hard
...
//TODO fix the noise problem and debug 
void main(){
	...
	char message[50] = "Angle(Degrees)	Distance(m)";
	float distances[90];
	...
	//this will maayyyybe work but its a strech.
