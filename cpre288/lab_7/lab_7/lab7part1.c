...
void objectDetection(float distances[], char message[], cyBOT_Scan_t *scan, int smallest[]){
    ...
    for (i = 3; i < 90 - 1; i++){
        //start object
        if ((distances[i] < 50.0) && (distances[i-1] - distances[i]) > 1.0){
            startAngle = i * 2;
            endAngle = startAngle;
    ...
    smallest[0] = smallestObjectDistance;
    return;
}
...
void cleanData(float uncleanData[], char message[], cyBOT_Scan_t *scan, int smallest[]){
    ...
    for (i = 0; i < 90 - 3; i++){
        ...
        if (fabs(uncleanData[i] - uncleanData[i + 1]) < bounds && uncleanData[i] < 49.0){
...
void main(){
    ...
    right_calibration_value = 269500; //calibrated_vals.right;
    ...
    while(input_byte != ESCAPE_KEY){
        ...
        if (input_byte == 'h'){
            input_byte = 'a';
            while (input_byte != 'h'){
                //do autonomous stuff until h is pressed again
