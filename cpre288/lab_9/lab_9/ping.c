...
void configure_timer(){
    ...
    //TIMER3_CTL_R = (TIMER3_CTL_R & 0xFFFFF0FF) | 0b110000000000;  //edge select both edges
...
unsigned int ping_read(void) {
    ...
    //wait for ISR to caputre rising edge & falling edge time
    while(state != DONE){}
...
void send_pulse(void){
    ...
    //GPIO_PORTB_DATA_R &= 0xFFFFFFF7; //set PB3 to low
